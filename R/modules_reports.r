modules.report.sheets <- function(env, spot.list, main, path)
{
  ...
  for (m in seq_along(spot.list$spots))
  {
    ...
    layout(matrix(c(1,2,4,1,3,4,5,5,6,7,7,6), 3, 4), widths=c(1,1,2,2), heights=c(2,1,1))
    ...
    text(0.1, 0.66, paste("# SNPs =", length(spot.list$spots[[m]]$genes)), adj=0)
    ...
    if (length(spot.list$spots[[m]]$genes) > 0 && length(spot.list$spots[[m]]$genes) < 5000)
    {
      ...
      maf.spot <- sapply(spot.list$spots[[m]]$genes, function(x)
        {
        snp <- unlist( env$indata.temp[x,sample.with.spot])
        return(sum(snp)/(length(snp)*2))
      })
      maf.all <- sapply(spot.list$spots[[m]]$genes, function(x)
      {
        snp <- unlist( env$indata.temp[x,])
        return(sum(snp)/(length(snp)*2))
      })
      ...
      if (main %in% c("Underexpression Spots"))
      {
        ...
        o <- names(sort(maf.spot, decreasing=FALSE)) #major allele freq
      }  else
      {
        #o <- names(sort(e.max, decreasing=TRUE))
        o <- names(sort(maf.spot, decreasing=TRUE))
      }
      ...
      text(0, 0.88, "Spot SNPlist", cex=1.8, adj=0)
      
      text(x.coords, rep(c(0.82, 0.80), 4)[1:7],
           c("Rank", "ID", "maf spot", "maf all", "r", "Symbol", "Description"),
           cex=1, adj=0)
      ...
      text(x.coords[3], y.coords, round(maf.spot[o], 2), cex=0.6, adj=0)
      text(x.coords[4], y.coords, round(maf.all[o], 2), cex=0.6, adj=0)
    ...
    # MAF Plot
    maf_mat <- cbind(maf.spot, maf.all)
    if(nrow(maf_mat) > 1)
    {
      par(mar=c(4,4,4,4))  
      barplot(t(maf_mat),col = c("#70916E", "#8F6E91"), main="MAF distribution ",
              names.arg= rep("",nrow(maf_mat)),
              las=2, cex.main=1, cex.lab=1, cex.axis=1, cex.names=0.8, beside = T,
              border=if (nrow(maf_mat) < 80) "black" else NA,
              ylab = "MAF")
      legend(x = "bottom", legend = c("MAF in spot", "MAF in data"), fill = c("#70916E", "#8F6E91"), ncol = 2,
             xpd=TRUE, inset=c(0, -.25), cex=.8)
      box()
    }else
    {
      plot.new()
    ...
    if (env$preferences$activated.modules$geneset.analysis)
    {
      ...
      text(0, 0.88, "SNPset Overrepresentation", cex=1.8, adj=0)
      
      text(x.coords, 0.82, c("Rank", "p-value", "#in/all", "SNPset", ""),
           cex=1, adj=0)
      ...
      {
        length(intersect(x$Snps, unique(env$gene.info$ensembl.mapping$refsnp_id[which(env$gene.info$ensembl.mapping[,1]%in%spot.list$spots[[m]]$genes)])   ))
      ...
      for (i in sort(unique(sapply(env$gs.def.list, function(x) { x$Type }))))
      {
        ...
        text(x.coords, 0.92, c("Rank", "p-value", "#in/all", "SNPset", ""), cex=0.8, adj=0)
