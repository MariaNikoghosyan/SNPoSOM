...
GeneSet.Fisher.Snp <- function(list.ids, all.ids, gs.def.list, sort=FALSE)
{
  list.ids <- list.ids[!is.na(list.ids)&!list.ids==""]
...
## function for  for SNPs
Sample.SSZ <- function(gene.set,ex.ensID.m,mean.ex.all.snp,sd.ex.all.snp)
{
  mean.ex.set <- colMeans( ex.ensID.m[gene.set$Snps,] )
  
  GSZ <- sqrt(length(gene.set$Snps)) * ( mean.ex.set - mean.ex.all.snp ) / sd.ex.all.snp
}
