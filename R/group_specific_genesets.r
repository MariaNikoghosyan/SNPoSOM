pipeline.groupSpecificGenesets <- function(env)
{
  ...
  for (gr in seq_along(unique(env$group.labels)))
  {
    ...
    top.gs <- gs.p.values[1:ifelse(length(gs.p.values) > 20 , 20, length(gs.p.values))] #changed
  ...
  top.gs <- names(sort(gs.F,decreasing=TRUE)[1:ifelse(length(gs.p.values) > 20 , 20, length(gs.p.values))]) #changed
  ...
  for (i in 1:ifelse(length(gs.p.values) > 20 , 20, length(gs.p.values))) #changed
  {
    ylim <- c(-15, 20)
    par(mar=c(3,5,2,1))
