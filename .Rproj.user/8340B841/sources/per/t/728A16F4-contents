pipeline.groupAnalysis <- function(env)
{
  ...
  local.env$indata.temp <- env$indata.temp
  local.env$preferences$group.maf <- TRUE
  local.env$group.labels.origin <- env$group.labels
...
#      p.g.m[,gr] <- order(apply(indata[,samples.indata,drop=FALSE],1,mean)) / nrow(indata)
  ...
  local.env$indata.temp <- do.call(cbind, by(t(env$indata.temp ),
                              env$group.labels,
                             colSums)[unique(env$group.labels)])
